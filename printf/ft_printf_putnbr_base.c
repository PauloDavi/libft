...
/*   Updated: 2025/03/30 17:00:32 by cobli            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *putnbr_base(unsigned long nbr, int len, const char *base, t_element element);
...
static int number_len_base(unsigned long nbr, int base_len, t_element element) {
  ...
  if (element.precision == -1 && !(element.flags & MINUS_FLAG_MASK) && (element.flags & ZERO_FLAG_MASK) && element.width > len + prefix_len)
