...
/*   Updated: 2025/03/30 17:00:12 by cobli            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int number_len(int n, t_element element) {
  ...
  if (element.precision >= len)
    return (element.precision + (is_neg || element.flags & (SPACE_FLAG_MASK & PLUS_FLAG_MASK)));
  if (element.precision == -1 && !(element.flags & MINUS_FLAG_MASK) && (element.flags & ZERO_FLAG_MASK) && element.width > len)
